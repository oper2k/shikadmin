...
class ShikAdminPanelSupabaseUser extends BaseAuthUser {
  ShikAdminPanelSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> shikAdminPanelSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ShikAdminPanelSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
